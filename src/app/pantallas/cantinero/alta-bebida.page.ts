  ...
  template: `
    <div class="lm-screen">
      ...
      <div class="lm-body lm-body--gap12">
        ...
        <section aria-labelledby="fotos-title"><span id="fotos-title" class="lm-label">Fotos de presentación</span><div class="fotos">@for (indice of [0,1,2]; track indice) { <lm-foto forma="rectangulo" [tamano]="104" [fuente]="fotos()[indice]" [etiqueta]="'Foto ' + (indice + 1)" (capturar)="cargarFoto(indice)" /> }</div>@if (errorFotos()) { <span class="lm-field__error">{{ errorFotos() }}</span> } @else { <span class="lm-field__hint">Agregá tres fotos con la cámara o desde la galería.</span> }</section>
      </div>
...
export class AltaBebidaPage {
  ...
  protected async guardar(): Promise<void> {
    ...
    const seguro = await this.confirmacion.pedir({ titulo: '¿Agregás esta bebida?', mensaje: 'Estará disponible en la carta.', confirmar: 'Agregar bebida', tono: 'exito', icono: 'local_bar', detalle: [{ rotulo: 'Bebida', valor: datos.nombre }, { rotulo: 'Tiempo', valor: `${datos.minutos} minutos` }] });
    ...
    try {
      ...
      this.avisos.exito('Bebida agregada', `${datos.nombre} ya está en la carta.`);
      await this.router.navigate(['/sector/pedidos'], { replaceUrl: true });
    } catch {
      this.avisos.error('No pudimos agregar la bebida', 'Intentá de nuevo. Los datos siguen en el formulario.');
    } finally {
      this.guardando.set(false);
    }
  }
  protected volver(): void { void this.router.navigate(['/sector/pedidos']); }
}
